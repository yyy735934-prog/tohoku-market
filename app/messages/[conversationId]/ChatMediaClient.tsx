...
type ChatInfo = { id: string; providerGroupId: string; counterpart: string; purpose: "trade" | "moderation"; viewerIsAdmin: boolean; listing: { id: string; title: string; price: number; status: string; icon: string; imageUrl: string | null } };
...
const MODERATION_PROMPTS = ["请补充一张商品实拍图。", "请确认商品的功能、成色和瑕疵情况。", "请说明品牌型号、配件及是否能正常使用。"];
...
export default function ChatMediaClient({ conversationId }: { conversationId: string }) {
  ...
  return <main className="chat-page">
    <header className="chat-topbar"><Link href={info?.viewerIsAdmin && info.purpose === "moderation" ? "/admin" : "/messages"} aria-label={info?.viewerIsAdmin ? "返回审核后台" : "返回消息"}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m14.5 6-6 6 6 6" /></svg></Link><div><b>{info?.purpose === "moderation" ? `${info.counterpart} · 商品核实` : info?.counterpart ?? "匿名交易聊天"}</b><small>{info?.purpose === "moderation" ? "平台审核专用会话 · 身份信息不公开" : "站内匿名 · 身份信息不公开"}</small></div></header>
    {info && <Link className="chat-listing" href={info.purpose === "moderation" ? (info.viewerIsAdmin ? "/admin" : "/account") : `/?listing=${encodeURIComponent(info.listing.id)}`}><div>{info.listing.imageUrl ? <img src={info.listing.imageUrl} alt="" /> : info.listing.icon}</div><span><b>{info.listing.title}</b><small>{info.listing.price === 0 ? "免费赠送" : `¥${info.listing.price.toLocaleString()}`} · {info.purpose === "moderation" ? "待审核" : info.listing.status === "active" ? "在售" : info.listing.status === "sold" ? "已售出" : "已下架"}</small></span><i>{info.purpose === "moderation" ? (info.viewerIsAdmin ? "返回审核 ›" : "查看发布 ›") : "查看商品 ›"}</i></Link>}
    <section className="chat-messages" aria-live="polite"><p className="chat-privacy">{info?.purpose === "moderation" ? "此会话仅用于核实商品信息。请勿发送学生证、银行卡等无关敏感资料。" : "请勿发送证件、银行卡等敏感信息。语音可能被熟人辨认，建议在公共场所交易。"}</p>{info?.purpose === "moderation" && info.viewerIsAdmin && !messages.length && <div className="moderation-prompts"><b>常用核实话术</b>{MODERATION_PROMPTS.map((prompt) => <button key={prompt} type="button" onClick={() => setText(prompt)}>{prompt}</button>)}</div>}{messages.map((message) => <article key={message.id} className={`${message.mine ? "mine" : "theirs"} ${message.kind}`}><small>{message.mine ? "我" : info?.counterpart}</small>{message.kind === "text" && <p>{message.text}</p>}{message.kind === "image" && <button className="chat-image" type="button" onClick={() => setPreviewImage(message.mediaUrl!)} aria-label="查看图片大图"><img src={message.mediaUrl} alt="聊天图片" loading="lazy" /></button>}{message.kind === "audio" && <div className="chat-audio"><audio controls preload="metadata" src={message.mediaUrl} aria-label="播放语音消息" />{message.duration ? <span>{formatDuration(message.duration)}</span> : null}</div>}<time>{new Date(message.sentAt * 1000).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}</time></article>)}<div ref={bottomRef} /></section>
