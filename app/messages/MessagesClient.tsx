...
export default function MessagesClient() {
  ...
  return <section className="messages-shell">
    <header><span>站内匿名消息</span><h1>消息</h1><p>交易沟通与平台审核核实都在站内完成；不会向对方公开真实姓名、邮箱或第三方账号信息。</p></header>
    ...
    <div className="conversation-list">{items.map((item) => <Link key={item.id} href={`/messages/${item.id}`}>
      ...
      <div><div className="conversation-heading"><b>{item.counterpart}</b><small>{item.lastAt ? new Date(item.lastAt * 1000).toLocaleString("zh-CN", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" }) : ""}</small></div><strong>{item.listing.title}</strong><p>{item.lastText}</p><em>{item.purpose === "moderation" ? "审核核实" : item.listing.status === "active" ? "在售" : item.listing.status === "sold" ? "已售出" : "已下架"}</em></div>
