...
export default function MobileNav({ active, viewer = true, chatEnabled = true, homeHref = "/", onPublish }: MobileNavProps) {
  const messagesHref = viewer ? (chatEnabled ? "/messages" : "/favorites") : "/signin-with-chatgpt?return_to=%2Fmessages";
  ...
  return <nav className="mobile-nav" aria-label="移动端导航">
    ...
    <Link className={active === "messages" ? "active" : ""} href={messagesHref}>
      <span className="nav-envelope">{chatEnabled ? <EnvelopeIcon /> : "♡"}</span>{chatEnabled ? "消息" : "收藏"}
    </Link>
